...
enum OpCode {
     ...
     *	    else if B is greater than 1, the function has (B - 1) parameters,
     *	    and if B is 0 it means that the function parameters range from (A + 1) to the top of stack
     * C represents the number of return,
     *	    if C is 1, there is no value return,
     *	    else if C is greater than 1, then it has (C - 1) return values,
     *	    or if C is 0 the return values range from A to the top of stack
     */
    OP_CALL,

    /* A, B; return R[A], ... R[A + B - 2]
     * return the values to the calling function,
     * B represent the number of results.
     *	    if B is 1, that means no value return,
     *	    if B is greater than 1, it means there are (B - 1) values return.
     *	    And finally, if B is 0, the set of values range from R[A] to the top of stack,
    ...
    OP_EQ, // A B C; if (A ~= (RK(B) == RK(C))) then pc++
    OP_LT, // A B C; if (A ~= (RK(B) <  RK(C))) then pc++
    OP_LE, // A B C; if (A ~= (RK(B) <= RK(C))) then pc++
