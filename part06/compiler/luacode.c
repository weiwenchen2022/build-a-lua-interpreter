...
static int validop(FuncState *fs, const TValue *v1, const TValue *v2, int op)
{
    ...
    case LUA_OPT_IDIV: case LUA_OPT_MOD:
    case LUA_OPT_SHL: case LUA_OPT_SHR:
	if (!ttisinteger(v1) || !ttisinteger(v2))
	    return 0;

	if (LUA_OPT_IDIV == op && v2->value_.i == 0)
	    return 0;

	return 1;

    case LUA_OPT_UMN:
    case LUA_OPT_ADD: case LUA_OPT_SUB:
    case LUA_OPT_MUL: case LUA_OPT_DIV: case LUA_OPT_POW: {
	lua_State *L = fs->ls->L;
	lua_Number n1, n2;
	if (!luaV_tonumber(L, v1, &n1) || !luaV_tonumber(L, v2, &n2))
	    return 0;

	if (LUA_OPT_DIV == op && n2 == 0.0f)
	    return 0;
...
void luaK_infix(FuncState *fs, int op, expdesc *e)
{
    ...
    case BINOPR_IDIV: case BINOPR_MOD: case BINOPR_POW:
    case BINOPR_BAND: case BINOPR_BOR: case BINOPR_BXOR:
    case BINOPR_SHL: case BINOPR_SHR:
	if (!tonumeral(e, NULL))
	    luaK_exp2RK(fs, e);
    ...
    default:
	fprintf(stderr, "luaK_infix:unknow binopr: %d\n", op);
...
static void codecmp(FuncState *fs, int op, expdesc *e1, expdesc *e2)
{
    ...
    default:
	fprintf(stderr, "unknow compare operator: %d\n", op);
	luaD_throw(fs->ls->L, LUA_ERRPARSER);
	break;
    }
...
void luaK_posfix(FuncState *fs, int op, expdesc *e1, expdesc *e2)
{
	...
	*e1 = *e2;
    ...
    default:
	fprintf(stderr, "luaK_posfix:unknow binopr: %d\n", op);
	luaD_throw(fs->ls->L, LUA_ERRPARSER);
	break;
    }
}
...
int luaK_exp2RK(FuncState *fs, expdesc *e)
{
    ...
    case VTRUE: case VFALSE: e->u.info = luaK_boolK(fs, VTRUE == e->k); goto vk;
    case VK:
    vk:
	e->k = VK;
...
void luaK_indexed(FuncState *fs, expdesc *e, expdesc *key)
{
    ...
    e->u.ind.vt = VNONRELOC == e->k ? VLOCAL : VUPVAL;
    e->k = VINDEXED;
}
...
void luaK_setlist(FuncState *fs, int base, int nelemts, int tostore)
{
    ...
    int b = LUA_MULRET == tostore ? 0 : tostore;
...
static void discharge2reg(FuncState *fs, expdesc *e, int reg)
{
	...
	luaK_codeABx(fs, OP_LOADK, reg, luaK_boolK(fs, VTRUE == e->k));
	break;
...
static int exp2reg(FuncState *fs, expdesc *e, int reg)
{
	...
	if (need_value(fs, e->t) || need_value(fs, e->f)) {
	    int jf = VJMP == e->k ? NO_JUMP : luaK_jump(fs, e);
