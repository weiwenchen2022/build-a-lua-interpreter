...
static Node *mainposition(struct lua_State *L, struct Table *t, const TValue *key)
{
    switch (key->tt_) {
    case LUA_NUMINT:
	return hashint(key->value_.i, t);

    case LUA_NUMFLT:
	return hashint(l_hashfloat(key->value_.n), t);

    case LUA_TBOOLEAN:
	return hashboolean(key->value_.b, t);

    case LUA_SHRSTR:
	return hashstr(gco2ts(gcvalue(key)), t);
...
const TValue *luaH_getint(struct lua_State *L, struct Table *t, int key)
{
    ...
    for (;;) {
	const TKey *k = &n->key;
	if (ttisinteger(&k->tvk) && key == k->tvk.value_.i)
...
{
    lua_assert(LUA_SHRSTR == key->tt_);

    struct Node *n = hashstr(key, t);

    for (;;) {
	TKey *k = &n->key;
	if (ttisshrstr(&k->tvk) && luaS_eqshrstr(L, key, gco2ts(gcvalue(&k->tvk))))
	    return getval(n);
...
const TValue *luaH_get(struct lua_State *L, struct Table *t, const TValue *key)
{
    ...
    case LUA_NUMINT:
	return luaH_getint(L, t, key->value_.i);

    case LUA_NUMFLT:
	return luaH_getint(L, t, l_hashfloat(key->value_.n));

    case LUA_SHRSTR:
	return luaH_getshrstr(L, t, gco2ts(gcvalue(key)));

    case LUA_LNGSTR:
	return luaH_getstr(L, t, gco2ts(gcvalue(key)));
...
/* nums[i]统计array (2^(i - 1), 2^i]区间内, 不为nil的元素个数
...
// nums[i] = number of keys 'k' where 2^(i-1) < k && k <= 2^i
...
TValue *luaH_newkey(struct lua_State *L, struct Table *t, const TValue *key)
{
	...
	if (other_node != main_node) {
	    ...
	    while (main_node != other_node + other_node->key.nk.next)
...
static unsigned int findindex(struct lua_State *L, struct Table *t, const TValue *key)
{
	 ...
	 */
	if (luaV_eqobject(L, key, getkey(n))
