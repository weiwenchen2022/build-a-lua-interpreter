...
static lu_mem traversethread(struct lua_State *L, struct lua_State *th)
{
    ...
    return sizeof(struct lua_State) + th->stack_size * sizeof(TValue) + th->nci * sizeof(struct CallInfo);
}
...
static lu_mem traverse_strong_table(struct lua_State *L, struct Table *t)
{
    ...
    return sizeof(struct Table) + t->arraysize * sizeof(TValue) + twoto(t->lsizenode) * sizeof(Node);
}
...
static inline void setdebt(struct lua_State *L, l_mem debt)
{
    struct global_State *g = G(L);
...
// again
static inline void setpause(struct lua_State *L)
{
    struct global_State *g = G(L);
