...
#define vkisvar(v) (VLOCAL <= v->k && v->k <= VINDEXED)
#define check_condition(ls, c, s) if (!(c)) luaX_syntaxerror(ls->L, ls, s)
#define hasmulret(v) (VCALL == (v)->k)
...
{
    singlevaraux(fs, e, n);
    if (VVOID == e->k) { // variable is in global
	singlevaraux(fs, e, fs->ls->env);
	lua_assert(VUPVAL == e->k);
	expdesc k;
	codestring(fs, n, &k);
	luaK_indexed(fs, e, &k);
    }
...
/* This structure is used for recording information in brace
 */
struct ConsControl {
...
static void closelistfield(FuncState *fs, struct ConsControl *cc)
{
    ...
    if (LFIELD_PER_FLUSH == cc->tostore) {
...
static void recfield(FuncState *fs, struct ConsControl *cc)
{
    ...
    default:
	luaX_syntaxerror(fs->ls->L, fs->ls, "unknow token for recfield");
    }
...
static void exprstat(struct lua_State *L, LexState *ls, FuncState *fs)
{
	...
	check_condition(ls, VCALL == lh.v.k, "exp type error");
}
