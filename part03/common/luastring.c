...
struct TString *luaS_new(struct lua_State *L, const char *str, unsigned int l)
{
	...
	if (((LUA_SHRSTR == ts->tt_ && l == ts->shrlen)
		    || (LUA_LNGSTR == ts->tt_ && l == ts->u.lnglen))
...
{
    return a == b || (a->shrlen == b->shrlen && memcmp(getstr(a), getstr(b), a->shrlen) == 0);
}

int luaS_eqlngstr(struct lua_State *L, struct TString *a, struct TString *b)
{
    return a == b || (a->u.lnglen == b->u.lnglen && memcmp(getstr(a), getstr(b), a->u.lnglen) == 0);
}
